/*=========================================================================
 * OUTBREAK SHIELD - Windows Console Port
 ...
 *
 * Ported from Mellivora OS (NASM x86 assembly) to Win32 Console C.
 *
 * Build with MinGW:
 *   i686-w64-mingw32-gcc -O2 -o outbreak.exe outbreak_win.c
 *   (or: x86_64-w64-mingw32-gcc -O2 -o outbreak.exe outbreak_win.c)
 *
 * Build with MSVC:
 *   cl /O2 outbreak_win.c /Fe:outbreak.exe
 *=========================================================================*/
...
/* VGA colors (Windows console attribute values match VGA) */
#define C_BLACK     0x00
#define C_BLUE      0x01
#define C_GREEN     0x02
#define C_CYAN      0x03
...
/* Defaults */
#define DEF_VACCINES    50
#define DEF_SUPPLIES    40
#define DEF_MORALE      70
...
#define BLOCK_SHADE '\xB0'
#define BLOCK_DOT   '\xFE'
...
static const char *howto_lines[] = {
    ...
    "Each month you get 3 actions. Customize settings from the menu!",
    " ",
    "ACTIONS:",
    ...
    "  * High morale reduces death rates",
...
/*-----------------------------------------------------------------------
 * Console helpers - replacements for Mellivora OS syscalls
 *-----------------------------------------------------------------------*/
...
static void print_str(const char *s) {
    DWORD written;
    WriteConsoleA(hConsole, s, (DWORD)strlen(s), &written, NULL);
}
...
static void do_sleep(int ticks) {
    /* Mellivora ticks are roughly 1/18.2s (~55ms each) */
    Sleep(ticks * 55);
}
...
static void draw_game_screen(void) {
    ...
    set_cursor(2, 12);
    set_color(C_WHITE);
    print_str("Outbreak Timeline: [Month 1");
    put_char('\xC4'); put_char('\xC4'); put_char('\xC4'); put_char('\xC4');
    print_str("12]");
...
static void custom_settings(void) {
    ...
    set_cursor(5, 3);
    set_color(C_LGREEN);
    print_str("Starting Vaccines: [1] 25  [2] 50  [3] 75  [4] 100");
    for (;;) {
        ch = get_char();
        if (ch == '1') { set_vaccines = 25; break; }
        if (ch == '2') { set_vaccines = 50; break; }
        if (ch == '3') { set_vaccines = 75; break; }
        if (ch == '4') { set_vaccines = 100; break; }
    }
    ...
    set_cursor(5, 5);
    set_color(C_LCYAN);
    print_str("Starting Supplies: [1] 20  [2] 40  [3] 60  [4] 80");
    for (;;) {
        ch = get_char();
        if (ch == '1') { set_supplies = 20; break; }
        if (ch == '2') { set_supplies = 40; break; }
        if (ch == '3') { set_supplies = 60; break; }
        if (ch == '4') { set_supplies = 80; break; }
    }
...
static void show_settings(void) {
    ...
    for (;;) {
        ...
        set_cursor(7, 17);
        set_color(C_LGREEN);
        print_str("[1] Easy   - 75 vaccines, 60 supplies, 90% morale");

        set_cursor(7, 18);
        set_color(C_YELLOW);
        print_str("[2] Normal - 50 vaccines, 40 supplies, 70% morale");

        set_cursor(7, 19);
        set_color(C_LRED);
        print_str("[3] Hard   - 25 vaccines, 20 supplies, 50% morale");
        ...
        if (ch == '1') {
            set_vaccines = 75; set_supplies = 60;
            set_morale = 90; set_months = 12; set_diff = 0;
            do_beep(SND_GOOD, 2);
            clear_screen();
        } else if (ch == '2') {
            ...
            set_vaccines = 25; set_supplies = 20;
            set_morale = 50; set_months = 12; set_diff = 2;
            do_beep(SND_ALARM, 2);
            clear_screen();
        } else if (ch == '4') {
            custom_settings();
...
static void title_screen(void) {
    ...
    put_char('\xC9');
    print_str("========================================");
    put_char('\xCD'); put_char('\xCD'); put_char('\xCD'); put_char('\xCD');
    put_char('\xCD'); put_char('\xCD'); put_char('\xCD'); put_char('\xCD');
    print_str("============");
    put_char('\xCD'); put_char('\xCD'); put_char('\xCD'); put_char('\xCD');
    put_char('\xBB');
    ...
    set_color(C_LGRAY);
    set_cursor(5, 15);
    print_str("The year is 2024. A deadly virus called Ratel Fever has");
    set_cursor(5, 16);
...
int main(void) {
    ...
    /* Set Code Page 437 for block characters */
    SetConsoleOutputCP(437);
    SetConsoleCP(437);

    /* Set console window size to 80x25 */
    windowSize.Left = 0;
    windowSize.Top = 0;
    windowSize.Right = 79;
    ...
    /* Set title */
    SetConsoleTitleA("Outbreak Shield - A Vaccination Simulation Game");
